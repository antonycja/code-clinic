...
def gen_creds(username):
    ...
    if LogIn.check_token(save_path(dir, ".logIn_token.json"), data, folders):
        ...
        if exists(save_path(folders['auth'], '.creds.token')):
            token = setup.decrypt_it(
                folders, "keys", "token", "creds", "token")
        ...
        creds = authentication.authenticate(
            save_path(dir, ".elite.json"), save_path(dir, ".creds.json"))

        # writing an encrypted version of the token
        if write_token:
            with open(save_path(dir, '.creds.json'), 'r') as file:
                enc_data = file.read()
            setup.encrypt_it(enc_data, folders, "keys", "token",
                             "SOS", "token", "creds", "token")
...
def get_profile():
    ...
    if files.check_user_profile(username):
        ...
        exit(
            f"profile '{username}' does not exists. run code-clinic configure")
...
def configure(name: str = None, email: str = None):
    ...
    setup.encrypt_it(data, folders, 'keys', 'creds',
                     'SOS', 'recon', 'config', 'creds')
    cs = authentication.get_credentials()
    setup.encrypt_it(cs, folders, 'keys', 'elite',
                     'cs', 'recon', 'cs', 'elite')
...
def login(log_file=save_path(files.get_home(), '.elite', '.systems.log')):
    ...
    log_data = dict()
    if exists(log_file):
        log_data = writer.load_pickle(
            save_path(files.get_home(), '.elite'), '.systems.log')
...
def signin(ctx):
    ...
    writer.capture_pickle(save_path(files.get_home(), '.elite'),
                          ".systems", "log", {"username": f'{username}'})
...
def make_booking(day, time, desc):
    ...
    signal, message = booking.book_slot(
        creds, booking_info, user_data['email'])
...
def cancel_booking(day: str, time: str):
    ...
    if confirm_cancellation():
        signal, message = booking.cancel_booking(
            creds, booking_info, user_data['email'])
    else:
...
def volunteer(day: str, time: str, campus: str):
    ...
    username = current_logged_profile()["username"]
    creds, user_data = gen_creds(username)
    message = volunteering.create_volunteer_slot(
        creds, user_data['email'], start_time, end_time, campus)
    exit(message)
...
def cancel_volunteering(day: str, time: str):
    ...
    if confirm_cancellation():
        message = volunteering.cancel_event(
            creds, start_time, end_time, user_data["email"])
    else:
        message = "Aborting cancellation!"
    exit(message)
